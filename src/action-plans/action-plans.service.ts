...
export class ActionPlansService {
  ...
  async create(createActionPlanDto: CreateActionPlanDto, userId: string): Promise<ActionPlanWithRelations> {
...
data: {
  ...
  start_date: actionPlanData.start_date ? new Date(actionPlanData.start_date) : null,
  due_date: actionPlanData.due_date ? new Date(actionPlanData.due_date) : null,
  ...
  async findOne(id: string, userId: string): Promise<ActionPlanWithRelations> {
    ...
    if (actionPlan.user_id !== userId) {
      throw new ForbiddenException('You do not have permission to access this action plan');
    }
  ...
  async update(
    ...
    const data: Prisma.action_planUpdateInput = {
      ...
      ...(updateData.start_date !== undefined && { 
        start_date: updateData.start_date ? new Date(updateData.start_date) : null 
      }),
  ...
  private async verifyOwnership(planId: string, userId: string): Promise<void> {
    ...
    if (actionPlan.user_id !== userId) {
      throw new ForbiddenException('You do not have permission to modify this action plan');
    }
  }
  ...
  private async updateGoals(
    ...
    const goalsToUpdate = goals.filter((g): g is { id: string } & typeof g => 
  !!g.id && existingGoalIds.includes(g.id));
    
    // Identificar metas para criar (não têm ID ou o ID não existe no banco)
    const goalsToCreate = goals.filter(g => !g.id || !existingGoalIds.includes(g.id));
      ...
      transaction.push(
        this.prisma.goal.update({
          ...
          data: {
            ...
            start_date: goalData.start_date ? new Date(goalData.start_date) : null,
...
async generateFromDiagnostic(diagnosticId: string, userId: string): Promise<ActionPlanWithRelations> {
  ...
  try {
    ...
    const insights = Array.isArray(diagnostic.insights) ? diagnostic.insights : [];
    const recommendations = Array.isArray(diagnostic.recommendations) ? diagnostic.recommendations : [];
    const areasFocus = Array.isArray(diagnostic.areas_focus) ? diagnostic.areas_focus : [];
    ...
    const diagnosticSummary = {
      ...
      score: typeof diagnostic.score_intelligent === 'number' ? diagnostic.score_intelligent : 0
    };
    ...
    const planoIA = await generateActionPlan({
      ...
      diagnostico: {
        ...
        dataGeracao: diagnostic.generated_at ? new Date(diagnostic.generated_at).toISOString() : new Date().toISOString(),
        dataConclusao: diagnostic.completed_at ? new Date(diagnostic.completed_at).toISOString() : null,
    ...
    actionPlanData.title = planoIA.titulo || `Plano de Ação - ${diagnostic.questionnaire?.title || 'Diagnóstico'}`;
    actionPlanData.description = planoIA.descricao || `Plano gerado em ${new Date().toLocaleDateString('pt-BR')}`;
    ...
    actionPlanData.progress = typeof planoIA.progresso === 'number' ? planoIA.progresso : 0;
    ...
    if (Array.isArray(planoIA.tarefas)) {
      planoIA.tarefas.forEach((tarefa: any, index: number) => {
        goals.push({
          ...
          due_date: this.calculateDueDate(typeof tarefa.prazoDias === 'number' ? tarefa.prazoDias : 7)
        });
      });
    }
    ...
    if (goals.length === 0) {
      goals.push({
        ...
        description: 'Analisar os resultados do diagnóstico e estabelecer metas iniciais de tratamento',
        status: 'pendente' as const,
...
private mapToValidCategory(category: string): 'wellness' | 'leadership' | 'development' | 'performance' | 'career' {
  const validCategories = ['wellness', 'leadership', 'development', 'performance', 'career'] as const;
  const normalizedCategory = category.toLowerCase();
  return validCategories.includes(normalizedCategory as any) ? normalizedCategory as 'wellness' | 'leadership' | 'development' | 'performance' | 'career' : 'wellness';
}
...
private async createBasicActionPlan(diagnostic: any, userId: string): Promise<ActionPlanWithRelations> {
  const actionPlanData = new CreateActionPlanDto();
