...
export class ActivityTrackingMiddleware implements NestMiddleware {
  ...
  async use(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    ...
    console.log(`Middleware: Tracking activity for user ${req.user.sub || req.user.id} on route ${req.path}`);
    ...
    res.on('finish', async () => {
      try {
        ...
        if (res.statusCode >= 200 && res.statusCode < 300) {
          ...
          console.log(`Middleware: Logging activity for user ${userId}, action: ${action}`);
          ...
          console.log(`Middleware: Activity logged successfully for user ${userId}`);

          // Verificações de segurança automáticas
          await this.auditService.checkOffHoursActivity(userId, action);
              await this.auditService.checkCriticalAction(userId, action, entityType || undefined);
        }
      } catch (error) {
  ...
  private getActionFromRequest(req: AuthenticatedRequest): string {
    ...
    if (path.includes('/questionnaires')) {
      switch (method) {
        case 'post': return 'create_questionnaire';
        case 'put':
        case 'patch': return 'update_questionnaire';
        case 'delete': return 'delete_questionnaire';
        case 'get': return 'view_questionnaire';
        default: return 'questionnaire_action';
      }
    }

    if (path.includes('/diagnostics')) {
      switch (method) {
        case 'post': return 'create_diagnostic';
        case 'put':
        case 'patch': return 'update_diagnostic';
        case 'delete': return 'delete_diagnostic';
        case 'get': return 'view_diagnostic';
        default: return 'diagnostic_action';
      }
    }
    ...
    if (path.includes('/permissions')) {
      switch (method) {
        case 'post': return 'create_permission';
        case 'put':
        case 'patch': return 'update_permission';
        case 'delete': return 'delete_permission';
        case 'get': return 'view_permission';
        default: return 'permission_action';
      }
    }
  ...
  private getEntityFromRequest(req: AuthenticatedRequest): { entityType: string | null, entityId: string | null } {
    const path = req.path;
  ...
  private isValidId(segment: string): boolean {
    ...
    return /^[a-zA-Z0-9_-]{20,}$/.test(segment) || // CUID-like
  ...
  private isActionSegment(segment: string): boolean {
    const actionSegments = ['stats', 'export', 'download', 'activate', 'deactivate', 'reset'];
    return actionSegments.includes(segment.toLowerCase());
  }

  private isEntityType(segment: string): boolean {
    const entityTypes = [
      'users', 'questionnaires', 'diagnostics', 'reports', 
      'permissions', 'companies', 'subscriptions', 'notifications'
    ];
  ...
  private async logActivity(
    ...
    try {
      console.log(`Middleware: Creating activity log for user ${userId}, action: ${action}`);
