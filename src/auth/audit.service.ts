...
export class AuditService {
  ...
  async getAuditStats(period: '24h' | '7d' | '30d' = '24h'): Promise<AuditStats> {
    const now = new Date();
    ...
    try {
      ...
      const activityTimeline = await this.generateActivityTimeline(startDate, period);
      const loginTimeline = await this.generateLoginTimeline(loginStats, period);

      // Contar logins suspeitos e alertas críticos
      const suspiciousLogins = loginStats.filter(l => l.status === 'failed').length;
      const criticalAlerts = this.securityAlerts.filter(a => a.severity === 'critical' && a.status === 'new').length;
      ...
      const complianceRate = totalActivities > 0 ? 
        Math.max(0, 100 - (suspiciousActions / totalActivities * 100)) : 100;
  ...
  async updateSecurityAlertStatus(alertId: string, status: 'new' | 'investigating' | 'resolved') {
  ...
  async checkFailedLoginAttempts(email: string, ipAddress: string) {
    ...
    const current = this.failedLoginAttempts.get(key) || { count: 0, lastAttempt: now };
  ...
  async checkCriticalAction(userId: string, action: string, entityType?: string) {
    const criticalActions = ['delete_user', 'update_user', 'delete_permission', 'create_permission'];
  ...
  private async createSecurityAlert(alertData: Omit<SecurityAlert, 'id' | 'status' | 'createdAt'>) {
  ...
  private async generateActivityTimeline(startDate: Date, period: '24h' | '7d' | '30d') {
  ...
  private generateLoginTimeline(loginStats: any[], period: '24h' | '7d' | '30d') {
    const intervals = period === '24h' ? 24 : period === '7d' ? 7 : 30;
    const timeline: { time: string; successful: number; failed: number }[] = [];
    
    for (let i = 0; i < intervals; i++) {
      const successful = loginStats.filter(l => 
        l.status === 'success' && 
        this.isInInterval(l.login_at, i, period)
      ).length;
      
      const failed = loginStats.filter(l => 
        l.status === 'failed' && 
        this.isInInterval(l.login_at, i, period)
      ).length;
  ...
  private isInInterval(date: Date, interval: number, period: '24h' | '7d' | '30d'): boolean {
    ...
    return Math.floor((Date.now() - date.getTime()) / (24 * 60 * 60 * 1000)) === interval;
  }
